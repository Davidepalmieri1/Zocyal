...
begin
  ...
  return jsonb_build_object(
    ...
    'missions', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'difficulty', mission.difficulty, 'verification_mode', mission.verification_mode,
      'completed', completion.id is not null, 'completed_at', completion.completed_at
    ) order by
      case mission.difficulty when 'easy' then 1 when 'medium' then 2 else 3 end,
      completion.id is not null, mission.points, mission.created_at)
      from public.missions mission
      left join public.participant_mission_completions completion
        on completion.mission_id = mission.id and completion.participant_id = v_participant
    ...
    'rewards', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'reward_type', available.reward_type, 'threshold_points', available.threshold_points,
      'podium_position', available.podium_position, 'quantity_total', available.quantity_total,
      ...
      from (
        select reward.*,
          greatest(reward.quantity_total - count(all_redemption.id)::integer, 0) as quantity_remaining,
        ...
        group by reward.id, own_redemption.id, own_redemption.status, own_redemption.claim_code
      ) available
    ), '[]'::jsonb)
  );
end;
