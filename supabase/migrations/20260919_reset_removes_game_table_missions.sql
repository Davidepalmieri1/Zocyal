...
begin
  ...
  -- A game table creates a private reward mission. Remove the tables first to
  -- release their restrictive foreign key, then remove only their generated
  -- missions. Organizer-created missions and rewards remain untouched.
  delete from public.game_tables where event_code = v_event;
  delete from public.missions
  where event_code = v_event
    and code like 'game-table-%';
...
revoke all on function public.admin_reset_event_participants(text)
...
comment on function public.admin_reset_event_participants(text) is
  'Deletes participant runtime data and generated game-table missions for one event while preserving organizer configuration.';
