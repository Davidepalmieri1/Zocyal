      ...
      {drinkOpen && drinkOffer && (
        <div className="fixed inset-0 z-[110] flex items-end justify-center overflow-y-auto bg-black/80 px-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4 backdrop-blur-sm sm:items-center sm:py-6">
          <div role="dialog" aria-modal="true" aria-label="Offerta drink" className="premium-glass max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-[2rem] bg-[#0b070d]/95 p-6 text-center">
            ...
            {drinkOffer.status==="accepted"&&drinkOffer.sender_id===mioId&&<><p className="mt-4 text-xs font-black uppercase tracking-[.18em] text-green-300">Offerta accettata</p><h2 className="mt-2 text-2xl font-black">2 € di sconto sul secondo drink</h2><p className="mt-3 text-sm text-gray-400">Mostra il QR allo staff. È personale e utilizzabile una sola volta.</p>{drinkOffer.coupon_code&&<div className="mt-5 rounded-2xl bg-white p-5"><QRCodeSVG value={drinkOffer.coupon_code} size={200} level="H" marginSize={1} className="mx-auto h-auto w-full max-w-[200px]" aria-label={`QR coupon ${drinkOffer.coupon_code}`}/><p className="mt-3 text-xl font-black tracking-[.16em] text-black">{drinkOffer.coupon_code}</p></div>}</>}
