...
export default function DancePage(){
  ...
  const [loading,setLoading]=useState(true); const [busy,setBusy]=useState(""); const [message,setMessage]=useState(""); const [profileFeedback,setProfileFeedback]=useState(""); const [matchProfileCompleted,setMatchProfileCompleted]=useState(false); const [showMatchPrompt,setShowMatchPrompt]=useState(false)
  ...
  async function save(){
    ...
    try{
      ...
      if(!matchProfileCompleted)setShowMatchPrompt(true)
  ...
  function invitationIssue(dancer:Dancer){if(!dancer.available)return "NON DISPONIBILE";if(!Object.keys(skills).some(style=>dancer.skills[style]))return "NESSUNO STILE IN COMUNE";if(!(role==="both"||dancer.role==="both"||role!==dancer.role))return "RUOLI NON COMPATIBILI";return ""}
  ...
  <section className="mt-8"><h2 className="text-2xl font-black">Ballerini dell’evento</h2><p className="mt-2 text-sm text-white/45">Vedi chi è disponibile; per invitare servono almeno uno stile condiviso e ruoli compatibili.</p><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{loading?<div className="h-48 animate-pulse rounded-3xl bg-white/[.04]"/>:dancers.map(dancer=>{const issue=invitationIssue(dancer);return <article key={dancer.participant_id} className="rounded-3xl border border-white/10 bg-white/[.04] p-5"><div className="flex items-center gap-3">{dancer.participant?.avatar_url?<img src={dancer.participant.avatar_url} alt="" className="h-14 w-14 rounded-2xl object-cover"/>:<span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-300/10 text-2xl">♫</span>}<div><h3 className="text-xl font-black">{dancer.participant?.nickname||"Ballerino"}</h3><p className="text-xs uppercase text-orange-200">{dancer.role==='both'?'Leader · Follower':dancer.role}</p></div></div><div className="mt-4 flex flex-wrap gap-2">{Object.keys(dancer.skills).map(style=><span key={style} className={`rounded-full px-3 py-1 text-xs ${skills[style]?'bg-pink-400/15 text-pink-100':'bg-white/[.05] text-white/40'}`}>{labels.get(style)}</span>)}</div><button disabled={Boolean(busy)||Boolean(issue)} onClick={()=>void invite(dancer)} className="mt-5 w-full rounded-full border border-orange-300/30 bg-orange-300/10 px-4 py-3 text-sm font-black text-orange-100 disabled:border-white/10 disabled:bg-white/[.03] disabled:text-white/35">{issue||"INVITA A BALLARE"}</button></article>})}{!loading&&!dancers.length&&<p className="rounded-3xl border border-dashed border-white/15 p-8 text-white/40">Non ci sono ancora altri profili di ballo in questo evento.</p>}</div></section>
  {showMatchPrompt&&<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" aria-labelledby="match-profile-title" className="w-full max-w-md rounded-[2rem] border border-pink-300/25 bg-[#160d18] p-7 text-center shadow-2xl"><span className="text-5xl">♥</span><h2 id="match-profile-title" className="mt-4 text-3xl font-black">Vuoi creare anche il profilo match?</h2><p className="mt-3 leading-6 text-white/55">Rispondi al questionario per scoprire nuove affinità. È facoltativo: puoi restare soltanto in modalità ballo.</p><button type="button" onClick={()=>router.push(`/evento/${code}/questionario`)} className="premium-cta mt-6 w-full rounded-full px-6 py-4 font-black">CREA PROFILO MATCH</button><button type="button" onClick={()=>setShowMatchPrompt(false)} className="mt-3 w-full rounded-full border border-white/15 px-6 py-4 font-black text-white/65">RESTA IN MODALITÀ BALLO</button></section></div>}
  </div></main>
}
