...
export async function GET(){
  ...
  const {data,error}=await getSupabaseAdmin().from("events").select("code,name,venue,status,experience_mode,starts_at,ends_at,updated_at").order("updated_at",{ascending:false}).limit(200)
