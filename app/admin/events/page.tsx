...
type EventItem = {
  ...
  experience_mode: "standard" | "inclusive" | "caribbean"
          ...
          <section className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {events.map((event, index) => (
              <article key={event.code} className="premium-glass premium-card-lift premium-enter relative overflow-hidden rounded-[2rem] p-6" style={{ animationDelay: `${Math.min(index, 6) * 70}ms` }}>
                ...
                <p className={`mt-4 rounded-xl border p-3 text-[10px] font-black uppercase tracking-wider ${event.experience_mode === "inclusive" ? "border-pink-400/20 bg-pink-400/[.08] text-pink-300" : event.experience_mode === "caribbean" ? "border-orange-300/20 bg-orange-300/[.08] text-orange-200" : "border-white/[.07] bg-white/[.025] text-white/40"}`}>{event.experience_mode === "inclusive" ? "Evento inclusivo" : event.experience_mode === "caribbean" ? "Evento caraibico" : "Evento standard"}</p>
