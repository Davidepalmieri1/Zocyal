...
export default function RewardsAdminPage() {
  ...
  return <div className="flex min-h-screen bg-black text-white"><Sidebar/><main className="min-w-0 flex-1 px-4 pb-16 pt-20 lg:p-8"><div className="mx-auto max-w-6xl">
    ...
    {tab==="missions"&&<div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <section><h2 className="text-2xl font-black">Missioni pubblicate</h2><div className="mt-4 grid gap-3">{data?.missions.map(m=><article key={m.id} className="rounded-2xl border border-white/10 bg-white/[.04] p-5"><div className="flex justify-between gap-4"><div><h3 className="font-black">{m.title}</h3><p className="mt-1 text-sm text-zinc-400">{m.description}</p><p className="mt-3 text-sm text-orange-300">+{m.points} punti · {m.verification_mode==="manual"?"Staff":"Automatica"}</p></div><button disabled={busy} onClick={()=>toggle("mission",m.id,!m.active)} className={`h-10 rounded-full px-4 text-xs font-black ${m.active?"bg-green-400/15 text-green-300":"bg-zinc-700 text-zinc-300"}`}>{m.active?"ATTIVA":"SPENTA"}</button></div></article>)}</div></section>
    </div>}
    ...
    {tab==="rewards"&&<div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <section><h2 className="text-2xl font-black">Podio Top 3</h2><div className="mt-4 grid grid-cols-3 items-end gap-2">{podium.map(({place,reward,leader})=><article key={place} className={`rounded-t-3xl border border-white/10 p-3 text-center ${place===1?"order-2 min-h-56 bg-amber-400/15":place===2?"order-1 min-h-48 bg-zinc-300/10":"order-3 min-h-40 bg-orange-500/10"}`}><p className="text-3xl font-black">{place}°</p><p className="mt-3 font-bold">{leader?.nickname||"—"}</p><p className="text-sm text-zinc-400">{leader?`${leader.points_earned} pt`:"Nessun punteggio"}</p><p className="mt-4 text-xs text-pink-300">{reward?.name||"Premio da configurare"}</p></article>)}</div>
        <h2 className="mt-8 text-2xl font-black">Catalogo premi</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data?.rewards.map(r=><article key={r.id} className="rounded-2xl border border-white/10 bg-white/[.04] p-5"><h3 className="font-black">{r.name}</h3><p className="mt-2 text-sm text-zinc-400">{r.description}</p><p className="mt-4 text-sm">Quantità: {r.quantity_total} · Costo: {r.points_cost} pt</p><button onClick={()=>toggle("reward",r.id,!r.active)} className="mt-4 rounded-full border border-white/10 px-4 py-2 text-xs font-black">{r.active?"ATTIVO":"SPENTO"}</button></article>)}</div></section>
    </div>}
