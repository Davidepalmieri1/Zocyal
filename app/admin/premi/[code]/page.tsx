...
export default function RewardsAdminPage() {
  ...
  async function assignPodium(id:string, podium_position:number){setBusy(true);setError("");try{await mutate({resource:"reward",action:"assign_podium",id,event_code:code,podium_position},"PATCH");await load()}catch(e){setError(e instanceof Error?e.message:"Errore inatteso.")}finally{setBusy(false)}}
  ...
  const podium=[1,2,3].map(place=>({place, reward:data?.rewards.find(r=>r.active&&Number(r.podium_position)===place), leader:data?.leaderboard.find(l=>Number(l.rank_position)===place)}))
  ...
  return <div className="flex min-h-screen bg-black text-white"><Sidebar/><main className="min-w-0 flex-1 px-4 pb-16 pt-20 lg:p-8"><div className="mx-auto max-w-6xl">
    ...
    {tab==="rewards"&&<div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <section><h2 className="text-2xl font-black">Podio Top 3</h2><div className="mt-4 grid grid-cols-3 items-end gap-2">{podium.map(({place,reward,leader})=><article key={place} className={`rounded-t-3xl border border-white/10 p-3 text-center ${place===1?"order-2 min-h-56 bg-amber-400/15":place===2?"order-1 min-h-48 bg-zinc-300/10":"order-3 min-h-40 bg-orange-500/10"}`}><p className="text-3xl font-black">{place}°</p><p className="mt-3 font-bold">{leader?.nickname||"—"}</p><p className="text-sm text-zinc-400">{leader?`${Number(leader.points) || 0} pt`:"Nessun punteggio"}</p><p className="mt-4 text-xs text-pink-300">{reward?.name||"Premio da configurare"}</p></article>)}</div>
        <h2 className="mt-8 text-2xl font-black">Catalogo premi</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data?.rewards.map(r=><article key={r.id} className="rounded-2xl border border-white/10 bg-white/[.04] p-5"><div className="flex items-start justify-between gap-3"><h3 className="font-black">{r.name}</h3><span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${r.reward_type==="podium_position"?"bg-amber-400/15 text-amber-300":"bg-sky-400/15 text-sky-300"}`}>{r.reward_type==="podium_position"?`PODIO ${r.podium_position}°`:`SOGLIA ${r.threshold_points} PT`}</span></div><p className="mt-2 text-sm text-zinc-400">{r.description}</p><p className="mt-4 text-sm">Quantità: {r.quantity_total} · Costo: {r.points_cost} pt</p><div className="mt-4 flex flex-wrap gap-2">{[1,2,3].map(place=><button key={place} disabled={busy} onClick={()=>assignPodium(r.id,place)} className={`rounded-full border px-3 py-2 text-xs font-black ${Number(r.podium_position)===place&&r.active?"border-amber-300 bg-amber-300 text-black":"border-amber-300/30 text-amber-200"}`}>ASSEGNA {place}°</button>)}<button disabled={busy} onClick={()=>toggle("reward",r.id,!r.active)} className="rounded-full border border-white/10 px-4 py-2 text-xs font-black">{r.active?"ATTIVO":"SPENTO"}</button><button disabled={busy} onClick={()=>remove("reward",r.id)} className="rounded-full border border-red-400/30 px-4 py-2 text-xs font-black text-red-300">ELIMINA</button></div></article>)}</div></section>
    </div>}
