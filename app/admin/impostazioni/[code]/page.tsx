...
export default function SettingsPage(){
  ...
  useEffect(()=>{void load().catch(e=>setError(e instanceof Error?e.message:"Errore inatteso."))},[load])
  ...
  const link=typeof window==="undefined"?`/evento/${code}`:`${window.location.origin}/evento/${code}`
  return <div className="flex min-h-screen bg-black text-white"><Sidebar/><main className="min-w-0 flex-1 px-4 pb-16 pt-20 lg:p-8"><div className="mx-auto max-w-4xl">
    ...
    {event&&<form onSubmit={save} className="mt-7 grid gap-6">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[.05] p-6"><h2 className="text-xl font-black">Link partecipanti</h2><p className="mt-3 break-all rounded-xl bg-black/40 p-4 text-sm text-pink-300">{link}</p><button type="button" onClick={()=>void navigator.clipboard.writeText(link).then(()=>setSuccess("Link copiato."))} className="mt-3 rounded-xl border border-pink-400/30 px-4 py-3 font-black text-pink-200">COPIA LINK</button></section>
